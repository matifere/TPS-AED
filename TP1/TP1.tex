...
\begin{proc}{sumaDeHabitantes}{\In menoresDeCiudades : \TLista{Ciudad}, \In mayoresDeCiudades : \TLista{Ciudad}} 
    ...
    \requiere{\{ 
    	...
    	\yLuego 
    	\left( \paraTodo{i,j}{\ent_{\geq 0}}{ 0 \leq i,j < |menoresDeCiudades| \wedge menoresDeCiudades[i]_{0} = mayoresDeCiudades[j]_{0} }  \right) $
    	\}}
	...
	\asegura{\{\paraTodo{n,m}{\ent}{ $\left 
			...
			\wedge \\ 
			( ciudades[n] \in \res)
			\longrightarrow ( \left( ciudades[n]_{1} = menoresDeCiudades[n]_{1} + mayoresDeCiudades[m]_{1} \right) \wedge \\ \left( ciudades[n]_{0} = menoresDeCiudades[n]_{0} \right)  
			)$}\}} 
...
\begin{proc}{hayCamino}{\In distancias : \TLista{\TLista{\ent}}, \In desde : \ent, \In hasta : \ent}{\bool \{ } 
	...
	\asegura{\{ 
		...
		\existe{p}{\TLista{\ent}}{
			...
			\paraTodo{k}{\ent}{ 
				(0 \leq k < |p|-1) \yLuego (distancias[p[k]][p[k+1]] > 0)
			}
		}\}}
\end{proc}
...
\requiere{
	\{ $n>0 \ \wedge$
	\paraTodo{i,j}{\ent}{
		\left( 0 \leq i,j < |conexión| \right) \yLuego \left( \left( i = j \right) \longrightarrow \left( conexión[i][j] = 0 \right) \right) \wedge \left( conexión[i][j] = conexión[j][i] \right) \ $\wedge$ $\left($ 0 \leq conexión[i[j] \leq 1 ] $\right)$
	}
	\}
}
\asegura{\{ \existe{p}{\TLista{\TLista{\TLista{\ent}}}}{\paraTodo{i,j,k}{\ent}{\left( 0 \leq k < n-1 \right) \wedge \left( 0 \leq i,j < n \right) \yLuego \left( p[k] = \text{conexión} \right) \yLuego \\ \left( \text{conexión}[i][j] = multEntreMatrices(p[k], p[k+1]) \right)}} \}}
\}

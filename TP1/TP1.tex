...
\begin{proc}{grandesCiudades} {\In ciudades : \TLista{Ciudad}}{\TLista{Ciudad}\{ }
	...
	\asegura{\{ $ \paraTodo{i}{\ent}{
				...
				\implica
				\left( ciudades[i] \in \res \right) \right) \yLuego estaIncluida(res, ciudades)
			} $ \}}
\end{proc} \}
...
\pred{estaIncluida}{s : \TLista{Ciudad}, t : \TLista{Ciudad}}{
	\paraTodo{k}{Ciudad}{(k \in s) \implica (\# apariciones(e,s) \leq \# apariciones(e,t))}
}
...
\begin{proc}{sumaDeHabitantes}{\In menoresDeCiudades : \TLista{Ciudad}, \In mayoresDeCiudades : \TLista{Ciudad}}
	...
	\requiere{ $\{
			...
			\wedge
			noHayNombresRepetidos(menoresDeCiudades) \wedge \\ noHayNombresRepetidos(mayoresDeCiudades)
			\}$ }
...
\pred{matrizCuadradaSimetrica}{matriz: \TLista{\TLista{\ent}}}{
	\paraTodo{i,j,k}{\ent}{
		\left( \left( 0 \leq i,j,k < |matriz| \right) \implicaLuego \left( |matriz[k]| = |matriz| \right) \right)  \implicaLuego \\ \left( \left( i = j \right) \longrightarrow \left( matriz[i][j] = 0 \right) \right) \wedge \left( matriz[i][j] = matriz[j][i] \right)
	}
}
...
\begin{proc}{cantidadCaminosNSaltos}{\Inout conexión : \TLista{\TLista{\ent}}, \In n : \ent}{ \{
		...
		\asegura{
			$\{ \existe{p}{\TLista{\TLista{\TLista{\ent}}}}{ \left( p[0] = C_{0} \right) \implicaLuego \paraTodo{i,j,k}{\ent}{
						\left(\left( \left( 0 < k < n \right) \wedge \left( 0 \leq i,j < |conexi \acute{o} n| \right) \right)
						\implicaLuego \left( p[k][i][j] = multEntreMatrices(p[k-1], p[k-1])  \right)\right)
						\yLuego
						\left( conexi \acute{o} n[i][j] = multEntreMatrices(p[n-1], p[n-1]) \right) }  } \}$ \\
		}
...
\begin{proc}{caminoMinimo}{\In origen : \ent, \In destino : \ent, \In distancias : \TLista{\TLista{\ent}}}{\TLista{\ent} \{ }{
	...
	\aux{todosLosCaminos}{distancias : \TLista{\TLista{\ent}}, origen : \ent, destino : \ent}{\TLista{\TLista{\ent}}}{
		$
			\existe{!m}{\TLista{\TLista{\ent}}}{
				\left( |m| > 1 \right) \yLuego
				\existe{p}{\TLista{\ent}}{
					(
					\left( |p| > 1 \right) \yLuego (
					\left( p[0] = origen \right) \wedge
					\left( p[|p|-1] = destino \right) \wedge
					\paraTodo{k}{\ent}{
						(0 \leq k < |p|-1) \implicaLuego (distancias[p[k]][p[k+1]] > 0)
					}
					)
					) \implicaLuego \left( p \in m \right)
				}
			}
		$
	}

	\aux{menorCamino}{distancias : \TLista{\TLista{\ent}}, origen : \ent, destino : \ent}{\TLista{\ent}}{$
		\existe{p}{\TLista{\ent}}{
		\existe{i}{\ent}{\left(
		...
		\right)
		\implicaLuego \\
		\left( ((\sum_{n = 1}^{|todosLosCaminos(distancias, origen, destino)[i]|-1} distancias[n-1][n]) \leq $ \\ $(\sum_{n = 1}^{|todosLosCaminos(distancias, origen, destino)[j]|-1} distancias[n-1][n])) \implica $ \\ $
		\left( p = todosLosCaminos(distancias, origen, destino)[i] \right) \right)
		}}
		}

	$}

	\aux{existeCamino}{distancias : \TLista{\TLista{\ent}}, desde : \ent, hasta : \ent}{\bool}{{
				$

...
\begin{proc}{cantidadCaminosNSaltos}{\Inout conexión : \TLista{\TLista{\ent}}, \In n : \ent}{ \{ }
	...
	\requiere{
		$\{ n>0 \ \wedge
			\paraTodo{i,j}{\ent}{
				\left( 0 \leq i,j < |conexi \acute{o} n| \right) \implicaLuego \left( \left( i = j \right) \longrightarrow \left( conexi \acute{o} n[i][j] = 0 \right)  \wedge \left( conexi \acute{o} n[i][j] = conexi \acute{o} n[j][i] \right) \ \wedge \left( 0 \leq conexi \acute{o} n[i][j] \leq 1  \right) \right)
			}
			\}$
	}

	%de esto no estoy muy seguro pero creo que al menos esta mejor

	\asegura{
	$\{ \existe{p}{\TLista{\TLista{\TLista{\ent}}}}{\paraTodo{i,j,k}{\ent}{
				\left( \left( 0 \leq k < n-1 \right) \wedge \left( 0 \leq i,j < |conexi \acute{o} n| \right) \right) \implicaLuego
				\left( conexi \acute{o} n[i][j] = multEntreMatrices(p[k], p[k+1]) \right) }  } \}$ \\
		}

...
\begin{proc}{grandesCiudades} {\In ciudades : \TLista{Ciudad}}{\TLista{Ciudad}\{ }
	...
	\requiere{\{ \paraTodo{i,j}{\ent}{0 \leq i,j < |ciudades| \implicaLuego ciudades[i]_{0} \neq ciudades[j]_{0}}\}}
	
	%se permuto el implica luego
	
	\asegura{\{ $ \paraTodo{i}{\ent}{
			\left( 0 \leq i \textless |ciudades| \right) \yLuego
			 \left( \left( ciudades[i]_{1} \textgreater 50000 \right)
			 \implicaLuego 
...
\begin{proc}{sumaDeHabitantes}{\In menoresDeCiudades : \TLista{Ciudad}, \In mayoresDeCiudades : \TLista{Ciudad}} 
    ...
    \requiere{\{ 
    	...
    	\wedge
    	\left( \paraTodo{i}{\ent}{ 
    		\existe{!j}{\ent}{0 \leq i,j < |menoresDeCiudades| \longrightarrow menoresDeCiudades[i]_{0} = mayoresDeCiudades[j]_{0} }
    		}  \right) $
    	\}}
...
\begin{proc}{hayCamino}{\In distancias : \TLista{\TLista{\ent}}, \In desde : \ent, \In hasta : \ent}{\bool \{ } 
	\requiere{
		\{
		\paraTodo{i,j}{\ent}{
			\left( 0 \leq i,j,desde,hasta < |distancias| \right) \yLuego \left( \left( i = j \right) \longrightarrow \left( distancias[i][j] = 0 \right) \right) \wedge \left( distancias[i][j] = distancias[j][i] \right)
		}
		\}
	}
	...
	\asegura{\{ 
		...
		\existe{p}{\TLista{\ent}}{
			\left( |p| \geq 1 \right) \yLuego \left(  
...
\text{Luego $M_{2}$ contiene la cantidad de 2-saltos asociados a cada par i,j que se encuentre dentro de la matriz}
\\
...
\requiere{
	\{ $n>0 \ \wedge$
	\paraTodo{i,j}{\ent}{
		\left( 0 \leq i,j < |conexión| \right) \implicaLuego \left( \left( i = j \right) \longrightarrow \left( conexión[i][j] = 0 \right)  \wedge \left( conexión[i][j] = conexión[j][i] \right) \ \wedge \left( 0 \leq conexión[i[j] \leq 1  \right) \right)
	}
	\}
}

	%de esto no estoy uy seguro pero creo que al menos esta mejor

\asegura{\{ \existe{p}{\TLista{\TLista{\TLista{\ent}}}}{\paraTodo{i,j,k}{\ent}{
			\left( \left( 0 \leq k < n-1 \right) \wedge \left( 0 \leq i,j < |conexión| \right) \right) \implicaLuego
			 \left( \text{conexión}[i][j] = multEntreMatrices(p[k], p[k+1]) \right) }} \}}
\}
...
Ahora demuestro que:\\
\vspace{0.3cm}
2.  {\{I \land B\}} \text{S} {\{{I}\} \longleftrightarrow {I \land B} \implica \textit{wp(S, I)}
...
wp \left(S1;S2,P_{c}\right) \equiv wp\left(res:=0; i:=0, P_{c} \right) \equiv wp\left( res:=0,def(i) \yLuego P_{c}\space_{i}^{0} \right) \equiv wp(S_{2}, res = 0 \wedge 0 = 0)) \\ \equiv wp(S_{2}, \True \yLuego res = 0))\equiv def(res) \yLuego P_{c}\space_{res} ^{0}  \equiv 0 = 0 \equiv \True \\
